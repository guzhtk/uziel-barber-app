...
class MainActivity : AppCompatActivity() {
    ...
    private fun renderSchedule() {
        ...
        val curMonthPrefix = "${now.get(Calendar.YEAR)}-${pad(now.get(Calendar.MONTH) + 1)}"
        val thisMonthAppts = list.filter { it.day.startsWith(curMonthPrefix) }
        val uniqueCustomers = thisMonthAppts.map { it.name.trim() }.distinct().size
        val statsTv = TextView(this)
        statsTv.text = "החודש: ${thisMonthAppts.size} תורים, $uniqueCustomers לקוחות שונים"
        statsTv.setTextColor(0xFF712B13.toInt())
        statsTv.setTypeface(null, android.graphics.Typeface.BOLD)
        statsTv.textSize = 14f
        statsTv.setPadding(0, 0, 0, 16)
